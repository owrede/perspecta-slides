...
<body class="perspecta-slides transition-${frontmatter.transition || 'fade'} ${themeClasses}">
   ...
   * By default, renders footnotes on individual slides where they are referenced
   * The 'footnotes' layout is just an extra collected view of all footnotes
   */
  private renderFootnotes(slide: Slide, layout?: string): string {
    ...
    // Don't render in the footnotes section for footnotes layout 
    // (the footnotes layout renders them as main content, not in the footer area)
    if (layout === 'footnotes') {
      return '';
    }
