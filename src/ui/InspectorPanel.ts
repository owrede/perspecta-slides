...
class GradientEditorModal extends Modal {
  ...
  onOpen() {
    ...
    contentEl.createEl('h2', {
      text: `Edit Dynamic Gradient (${this.mode === 'light' ? 'Light' : 'Dark'} Mode)`,
    });
    ...
    new Setting(toggleContainer)
      ...
      .addToggle((toggle) =>
        toggle.setValue(this.restartAtSection).onChange((value) => {
      ...
      if (newColors.length < 2) {
        ...
        if (errorEl) {
          ...
          const error = contentEl.createDiv({
            cls: 'gradient-error',
            text: 'Please enter at least 2 valid hex colors (e.g., #ff0000)',
          });
...
export class InspectorPanelView extends ItemView {
  ...
  setOnSlideMetadataChange(
    callback: (slideIndex: number, metadata: Partial<SlideMetadata>) => void
  ) {
  ...
  private renderPresentationTab(container: HTMLElement) {
    ...
    new Setting(container).setName('Title').addText((text) => {
      text.setPlaceholder('').setValue(fm.title || '');
      text.inputEl.addEventListener('blur', () => {
        this.updateFrontmatter({ title: text.getValue() });
      });
    });

    new Setting(container).setName('Author').addText((text) => {
      text.setPlaceholder('Author Name').setValue(fm.author || '');
      text.inputEl.addEventListener('blur', () => {
    ...
    new Setting(container).setName('Date').addText((text) => {
      text.setPlaceholder('YYYY-MM-DD').setValue(fm.date || '');
      text.inputEl.addEventListener('blur', () => {
        this.updateFrontmatter({ date: text.getValue() });
      });
    });

    new Setting(container).setName('Aspect Ratio').addDropdown((dropdown) => {
    ...
    new Setting(container).setName('Header Left').addText((text) => {
      text.setPlaceholder('').setValue(fm.headerLeft || '');
      text.inputEl.addEventListener('blur', () => {
        this.updateFrontmatter({ headerLeft: text.getValue() });
      });
    });

    new Setting(container).setName('Header Middle').addText((text) => {
      text.setPlaceholder('').setValue(fm.headerMiddle || '');
      text.inputEl.addEventListener('blur', () => {
        this.updateFrontmatter({ headerMiddle: text.getValue() });
      });
    });

    new Setting(container).setName('Header Right').addText((text) => {
      text.setPlaceholder('').setValue(fm.headerRight || '');
      text.inputEl.addEventListener('blur', () => {
        this.updateFrontmatter({ headerRight: text.getValue() });
      });
    });

    new Setting(container).setName('Footer Left').addText((text) => {
      text.setPlaceholder('').setValue(fm.footerLeft || '');
      text.inputEl.addEventListener('blur', () => {
        this.updateFrontmatter({ footerLeft: text.getValue() });
      });
    });

    new Setting(container).setName('Footer Middle').addText((text) => {
      text.setPlaceholder('').setValue(fm.footerMiddle || '');
      text.inputEl.addEventListener('blur', () => {
    ...
    new Setting(container)
      ...
      .addText((text) => {
        text.setPlaceholder('path/to/overlay.png').setValue(fm.imageOverlay || '');
        text.inputEl.addEventListener('blur', () => {
  ...
  private renderThemeTab(container: HTMLElement) {
    ...
    new Setting(container).setName('Slide Transition').addDropdown((dropdown) => {
    ...
    const renderColorPickers = (mode: 'light' | 'dark') => {
      ...
      const defaultBodyColor =
        mode === 'light'
          ? themePreset?.LightBodyTextColor || '#333333'
          : themePreset?.DarkBodyTextColor || '#e0e0e0';
      ...
      new Setting(titlesPickerContainer)
        .addColorPicker((picker) =>
          picker
            .setValue(
              (mode === 'light' ? currentFm.lightTitleText : currentFm.darkTitleText) ||
                defaultTitleColor
            )
            .onChange((value) => {
              if (mode === 'light') {
      ...
      new Setting(bodyPickerContainer)
        .addColorPicker((picker) =>
          picker
            .setValue(
              (mode === 'light' ? currentFm.lightBodyText : currentFm.darkBodyText) ||
                defaultBodyColor
            )
            .onChange((value) => {
              if (mode === 'light') {
      ...
      new Setting(headerPickerContainer)
        .addColorPicker((picker) =>
          picker
            .setValue(
              (mode === 'light' ? currentFm.lightHeaderText : currentFm.darkHeaderText) ||
                defaultHeaderColor
            )
            .onChange((value) => {
              if (mode === 'light') {
      ...
      new Setting(footerPickerContainer)
        .addColorPicker((picker) =>
          picker
            .setValue(
              (mode === 'light' ? currentFm.lightFooterText : currentFm.darkFooterText) ||
                defaultFooterColor
            )
            .onChange((value) => {
              if (mode === 'light') {
      ...
      if (isDynamicForThisMode) {
        ...
        let colors: string[];
        const fmColors =
          mode === 'light' ? currentFm.lightDynamicBackground : currentFm.darkDynamicBackground;
        if (fmColors && fmColors.length > 0) {
          colors = fmColors;
        } else {
          const themeGradient =
            mode === 'light' ? themePreset?.LightBgGradient : themePreset?.DarkBgGradient;
          colors =
            themeGradient ||
            (mode === 'light'
        ...
        new Setting(bgPickerContainer)
          .addColorPicker((picker) =>
            picker
              .setValue(
                (mode === 'light' ? currentFm.lightBackground : currentFm.darkBackground) ||
                  defaultBgColor
              )
              .onChange((value) => {
                if (mode === 'light') {
  ...
  private renderHeadingColors(container: HTMLElement, mode: 'light' | 'dark') {
    ...
    if (customHeadings.length === 0) {
      ...
      helpText.setText(
        'Headlines use the Title color by default. Add custom colors for specific heading levels.'
      );
    }
  }
  ...
  private renderLayoutBackgrounds(container: HTMLElement, mode: 'light' | 'dark') {
    ...
    const layoutConfigs = [
      ...
      { label: 'Title Slides', lightKey: 'lightBgTitle' as const, darkKey: 'darkBgTitle' as const },
      {
        ...
        lightKey: 'lightBgSection' as const,
        darkKey: 'darkBgSection' as const,
      },
    ];
    ...
    if (customLayouts.length === 0) {
      const helpText = container.createDiv({ cls: 'setting-item-description' });
      helpText.setText(
        'Layouts use the theme Background color by default. Add custom backgrounds for specific slide types.'
      );
    }
  }
