...
export class CreateDemoModal extends Modal {
  ...
  constructor(app: App, onOk: () => void, onGoToDemo: () => void) {
    super(app);
    this.onOk = onOk;
  ...
  onOpen() {
    ...
    contentEl.createEl('p', {
      text: 'The default theme demo presentation has been created in "Perspecta Slides Demo" folder.',
    });
...
export class PerspectaSlidesSettingTab extends PluginSettingTab {
  ...
  private onCreateDemo:
    | ((modalOnOk: () => void, modalOnGoToDemo: () => void) => Promise<void>)
    | null = null;
  ...
  display(): void {
    ...
    containerEl.createEl('h1', {
      text: 'Perspecta Slides',
      cls: 'perspecta-slides-settings-title',
    });
  ...
  private displayPresentationSettings(containerEl: HTMLElement): void {
    ...
    new Setting(containerEl)
      ...
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.showThumbnailNavigator).onChange(async (value) => {
    ...
    new Setting(containerEl)
      ...
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.showInspector).onChange(async (value) => {
  ...
  private displayThemesSettings(containerEl: HTMLElement): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Folder in your vault where custom themes are stored. Each theme is a subfolder with theme.json and theme.css.'
      )
      .addText((text) =>
        text
          ...
          .onChange(async (value) => {
            ...
            if (this.plugin.themeLoader) {
              this.plugin.themeLoader.setCustomThemesFolder(
                this.plugin.settings.customThemesFolder
              );
    ...
    if (customThemes.length === 0) {
      ...
      for (const theme of customThemes) {
        ...
        themeMeta.createEl('span', {
          text: `Fonts: ${theme.template.TitleFont} / ${theme.template.BodyFont}`,
        });
        ...
        const deleteBtn = themeActions.createEl('button', {
          cls: 'perspecta-font-btn perspecta-font-btn-danger',
        });
        ...
        deleteBtn.addEventListener('click', async (e) => {
          ...
          if (
            confirm(
              `Delete theme "${theme.template.Name}"? This will remove the theme folder and all its files.`
            )
          ) {
            try {
  ...
  private displayFontsSettings(containerEl: HTMLElement): void {
    ...
    new Setting(addFontSection)
      .setName('Font name')
      .setDesc(
        'Enter the Google Fonts name (e.g., "Saira", "Open Sans", "Noto Sans"). You can also paste a specimen URL.'
      )
      .addText((text) =>
        text.setPlaceholder('Saira').onChange((value) => {
          fontNameOrUrl = value;
        })
      );
    ...
    new Setting(addFontSection)
      ...
      .setDesc(
        'Custom name for UI display (spaces allowed, e.g., "My Serif"). Leave empty to use the font name.'
      )
      .addText((text) =>
        text.setPlaceholder('Leave empty to use font name').onChange((value) => {
          displayName = value;
        })
      );

    new Setting(addFontSection).addButton((button) =>
      button
        ...
        .onClick(async () => {
          ...
          if (!FontManager.isGoogleFontsUrl(fontNameOrUrl)) {
            new Notice(
              'Invalid font name or URL. Please use a valid Google Fonts name (with spaces allowed).'
            );
            return;
          }
          ...
          if (!discovery) {
            ...
            new Notice(
              `Failed to discover font variants for "${parsedName}". Please check that the font exists on Google Fonts.`
            );
            return;
          }
          ...
          const modal = new FontDiscoveryModal(
            ...
            async (selectedWeights, selectedStyles) => {
              ...
              console.log('[FontSettings] Starting download with selections:', {
                selectedWeights,
                selectedStyles,
              });
    ...
    new Setting(addLocalFontSection)
      ...
      .setDesc(
        'Path to a folder containing .otf, .ttf, .woff, or .woff2 files (relative to vault root)'
      )
      .addText((text) =>
        text.setPlaceholder('sample-data/Fonts/MyFont').onChange((value) => {
          localFontPath = value;
        })
      );
    ...
    new Setting(addLocalFontSection)
      ...
      .addText((text) =>
        text.setPlaceholder('MyFont').onChange((value) => {
          localFontName = value;
        })
      );

    new Setting(addLocalFontSection).addButton((button) =>
      button
        ...
        .onClick(async () => {
          ...
          const result = await fontManager.cacheLocalFont(
            localFontPath.trim(),
            localFontName.trim() || undefined
          );
          console.log('[FontSettings] cacheLocalFont result:', result);
          console.log(
            '[FontSettings] Plugin settings fontCache after:',
          ...
          if (result) {
            ...
            fontManager.reloadCache(this.plugin.settings.fontCache || { fonts: {} });
            console.log(
              '[FontSettings] Cache reloaded. Fonts:',
              fontManager.getAllCachedFonts().map((f) => f.name)
            );
    ...
    if (cachedFonts.length === 0) {
      const emptyState = containerEl.createDiv({ cls: 'perspecta-slides-info-box' });
      emptyState.createEl('p', {
        text: 'No fonts downloaded yet. Add a Google Fonts URL above to get started.',
      });
    } else {
      ...
      for (const font of cachedFonts) {
        ...
        const deleteBtn = fontActions.createEl('button', {
          cls: 'perspecta-font-btn perspecta-font-btn-danger',
        });
        setIcon(deleteBtn, 'trash-2');
        ...
        deleteBtn.addEventListener('click', async () => {
          if (
            confirm(`Delete font "${font.displayName}"? This will remove the cached font files.`)
          ) {
  ...
  private displayExportSettings(containerEl: HTMLElement): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Include speaker notes when exporting presentations to HTML. Notes will be hidden but accessible via presenter view.'
      )
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.exportIncludeSpeakerNotes).onChange(async (value) => {
...
This creates an image slide.`,
  ...
  private displayDebugSettings(containerEl: HTMLElement): void {
    ...
    new Setting(containerEl)
      ...
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.debugSlideRendering).onChange(async (value) => {
    ...
    new Setting(containerEl)
      ...
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.debugFontHandling).onChange(async (value) => {
      ...
      new Setting(containerEl)
        ...
        .addToggle((toggle) =>
          toggle.setValue(enabled).onChange(async (value) => {
            ...
            this.plugin.settings.debugTopics[
              topic as keyof typeof this.plugin.settings.debugTopics
            ] = value;
    ...
    debugList.createEl('li', {
      text: 'Slide rendering: Element parsing, column auto-detection, layout logic',
    });
    debugList.createEl('li', {
      text: 'Font loading: Font file downloads, caching, CSS generation',
    });
    debugList.createEl('li', {
      text: 'Topic-specific logs: Detailed trace information for specific features',
    });
