...
export class PresentationView extends ItemView {
  ...
  async loadFile(file: TFile) {
    ...
    debug.log(
      'presentation-view',
      `Parsed presentation, slides count: ${this.presentation.slides.length}`
    );
    ...
    debug.log(
      'presentation-view',
      `loadFile completed. File: ${this.file?.path}, has presentation: ${!!this.presentation}`
    );
  }
...
More content here...`,
  ...
  private renderSlideHeader(container: HTMLElement, index: number) {
    ...
    if (!fm?.headerLeft && !fm?.headerMiddle && !fm?.headerRight) {
      return;
    }
  ...
  private async exportHTML() {
    try {
      ...
      console.log(
        '[PresentationView]   onExportHTML callback:',
        this.onExportHTML ? 'set' : 'not set'
      );
